...
2. **Hero** — `$ whoami` types itself once (~600ms); "John Fulkerson" with blinking cursor; badge: **Incoming Software Engineer @ CoStar Group** (the one colored element); `const focus` line; 1–2 sentence blurb; CTAs: "View work ↓" + `resume.pdf`; social icons. No headshot in hero.
